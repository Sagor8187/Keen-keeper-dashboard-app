        ...
        <div className="bg-gray-100 p-6 rounded-2xl shadow text-center">
          ...
          <div className="mt-2 flex justify-center gap-2">
            <div className="flex  justify-center gap-2">
              {info.tags.map((item) => (
                <p className="bg-green-200 text-green-800 px-3 py-1 rounded-full text-sm">
                  {item}
                </p>
              ))}
            </div>
          ...
          <p className="text-sm text-gray-500">Preferred: {info?.email}</p>
        </div>
      ...
      <div className="col-span-2 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-gray-100 p-4 rounded-xl text-center shadow">
            <h1 className="text-2xl font-bold">{info?.days_since_contact}</h1>
        ...
        <div className="bg-gray-100 p-5 rounded-xl shadow flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600">
              Connect every <span className="font-bold">{info?.goal} days</span>
            </p>
          </div>
        ...
        <div>
          <div className="grid grid-cols-3 gap-4">
            <button
              onClick={() => {
                handlehistory({ ...info, calling: "audio", date: currnet });

                toast.success(`Call with ${info.name}`, {
                  position: "top-center",
                  autoClose: 5000,
              ...
              className="bg-gray-100 p-4 rounded-xl shadow flex flex-col items-center gap-2"
            >
              <FaPhoneAlt /> Call
            </button>
            <button
              onClick={() => {
                handlehistory({ ...info, calling: "text", date: currnet });

                toast.success(`Text With ${info.name}`, {
                  position: "top-center",
                  autoClose: 5000,
                  hideProgressBar: false,
                  closeOnClick: false,
            ...
            <button
              onClick={() => {
                handlehistory({ ...info, calling: "video", date: currnet });

                toast.success(`Video With ${info.name}`, {
                  position: "top-center",
                  autoClose: 5000,
                  hideProgressBar: false,
                  closeOnClick: false,
